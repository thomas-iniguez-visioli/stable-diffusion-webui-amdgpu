...
def create_ui():
    ...
    if shared.cmd_opts.onnx:
                    ...
                    with gr.Tabs(elem_id="onnx_tabs"):
                        with gr.Tab(label="Download ONNX Model"):
                            onnx_model_id = gr.Textbox(
                                label="ONNX Model ID (huggingface)",
                                value="runwayml/stable-diffusion-v1-5",
                                elem_id="onnx_model_id",
                            )
                        ...
                        with gr.Tab(label="Advanced (experimental)"):
                            ...
                            with gr.Column(elem_id="onnx_device_map"):
                                onnx_ep_text_encoder = gr.Textbox(
                                    label="Execution Provider for Text Encoder",
                                    value="DmlExecutionProvider",
                                    elem_id="onnx_ep_text_encoder",
                                )
                                onnx_id_text_encoder = gr.Textbox(
                                    label="Device ID for Text Encoder",
                                    value=(shared.cmd_opts.device_id or 0),
                                    elem_id="onnx_id_text_encoder",
                                )
                                onnx_ep_text_encoder_2 = gr.Textbox(
                                    label="Execution Provider for Text Encoder 2",
                                    value="DmlExecutionProvider",
                                    elem_id="onnx_ep_text_encoder_2",
                                )
                                onnx_id_text_encoder_2 = gr.Textbox(
                                    label="Device ID for Text Encoder 2",
                                    value=(shared.cmd_opts.device_id or 0),
                                    elem_id="onnx_id_text_encoder_2",
                                )
                                onnx_ep_unet = gr.Textbox(
                                    label="Execution Provider for UNet",
                                    value="DmlExecutionProvider",
                                    elem_id="onnx_ep_unet",
                                )
                                onnx_id_unet = gr.Textbox(
                                    label="Device ID for UNet",
                                    value=(shared.cmd_opts.device_id or 0),
                                    elem_id="onnx_id_unet",
                                )
                                onnx_ep_vae_decoder = gr.Textbox(
                                    label="Execution Provider for VAE Decoder",
                                    value="DmlExecutionProvider",
                                    elem_id="onnx_ep_vae_decoder",
                                )
                                onnx_id_vae_decoder = gr.Textbox(
                                    label="Device ID for VAE Decoder",
                                    value=(shared.cmd_opts.device_id or 0),
                                    elem_id="onnx_id_vae_decoder",
                                )
                                onnx_ep_vae_encoder = gr.Textbox(
                                    label="Execution Provider for VAE Encoder",
                                    value="DmlExecutionProvider",
                                    elem_id="onnx_ep_vae_encoder",
                                )
                                onnx_id_vae_encoder = gr.Textbox(
                                    label="Device ID for VAE Encoder",
                                    value=(shared.cmd_opts.device_id or 0),
                                    elem_id="onnx_id_vae_encoder",
                                )

                            button_onnx_device_map = gr.Button(
                                value="Apply",
                                variant="primary",
                                elem_id="button_onnx_device_map",
                            )
                        ...
                        button_onnx_device_map.click(
                            fn=save_device_map,
                            inputs=[
                                onnx_ep_text_encoder,
                                onnx_id_text_encoder,
                                onnx_ep_text_encoder_2,
                                onnx_id_text_encoder_2,
                                onnx_ep_unet,
                                onnx_id_unet,
                                onnx_ep_vae_decoder,
                                onnx_id_vae_decoder,
        ...
        from modules.sd_olive_ui import (
            optimize_sd_from_ckpt,
            optimize_sd_from_onnx,
            optimize_sdxl_from_ckpt,
            optimize_sdxl_from_onnx,
            available_sampling_methods,
        )

        global source, is_sdxl
        source = "ckpt"
        is_sdxl = False
        ...
        with gr.Blocks(analytics_enabled=False) as olive_interface:
            with gr.Row().style(equal_height=False):
                with gr.Column(variant="panel"):
                    with gr.Tabs(elem_id="olive_tabs"):
                        ...
                        with gr.Tab(label="Optimize checkpoint") as olive_tab_ckpt:
                            olive_checkpoint = gr.Textbox(
                                label="Checkpoint file name",
                                value="",
                                elem_id="olive_checkpoint",
                                info="Your own checkpoint file name",
                            )
                            olive_ckpt_vae = gr.Textbox(
                                label="VAE Source Model ID (huggingface)",
                                value="",
                                elem_id="olive_ckpt_vae",
                                info="The VAE from this model will be used. (empty for default)",
                            )
                            olive_ckpt_vae_subfolder = gr.Textbox(
                                label="VAE Source Subfolder",
                                value="vae",
                                elem_id="olive_ckpt_vae_subfolder",
                                info="The name of directory which has config and binary of the VAE. (empty for root)",
                            )
                            olive_ckpt_source_dir = gr.Textbox(
                                label="ONNX model folder name",
                                value="stable-diffusion-v1-5",
                                elem_id="olive_ckpt_source_dir",
                            )
                            olive_ckpt_outdir = gr.Textbox(
                                label="Output folder name",
                                value="stable-diffusion-v1-5",
                                elem_id="olive_ckpt_outdir",
                            )

                            with gr.Column(elem_id="olive_ckpt_res"):
                                olive_ckpt_sync_hw = gr.Checkbox(
                                    label="Sync height and width (RECOMMENDED)",
                                    value=True,
                                    elem_id="olive_ckpt_sync_hw",
                                )
                                olive_ckpt_sample_size = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Image Size",
                                    value=512,
                                    visible=olive_ckpt_sync_hw.value,
                                    elem_id="olive_onnx_sample_size",
                                )
                                olive_ckpt_sample_height = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Height",
                                    value=512,
                                    visible=not olive_ckpt_sync_hw.value,
                                    elem_id="olive_ckpt_sample_height",
                                )
                                olive_ckpt_sample_width = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Width",
                                    value=512,
                                    visible=not olive_ckpt_sync_hw.value,
                                    elem_id="olive_ckpt_sample_width",
                                )

                                olive_ckpt_sync_hw.change(
                                    onchange,
                                    inputs=[olive_ckpt_sync_hw],
                                    outputs=[
                                        olive_ckpt_sample_size,
                                        olive_ckpt_sample_height,
                                        olive_ckpt_sample_width,
                                    ],
                                )
                            ...
                            with FormRow(
                                ...
                                olive_ckpt_sampling_method = gr.Dropdown(
                                    choices=available_sampling_methods,
                                    value="euler",
                                    label="Sampling Method",
                                    elem_id="olive_ckpt_sampling_method",
                                )

                        with gr.Tab(label="Optimize ONNX model") as olive_tab_onnx:
                            olive_onnx_model_id = gr.Textbox(
                                label="ONNX Model ID (huggingface)",
                                value="runwayml/stable-diffusion-v1-5",
                                elem_id="olive_onnx_model_id",
                            )
                            olive_onnx_vae = gr.Textbox(
                                label="VAE Source Model ID (huggingface)",
                                value="",
                                elem_id="olive_onnx_vae",
                                info="The VAE from this model will be used. (empty for default)",
                            )
                            olive_onnx_vae_subfolder = gr.Textbox(
                                label="VAE Source Subfolder",
                                value="vae",
                                elem_id="olive_ckpt_vae_subfolder",
                                info="The name of directory which has config and binary of the VAE. (empty for root)",
                            )
                            olive_onnx_indir = gr.Textbox(
                                label="Input folder name",
                                value="stable-diffusion-v1-5",
                                elem_id="olive_onnx_indir",
                                info="If this folder exists, Olive will load and optimize model from it. Otherwise, download and optimize model on it.",
                            )
                            olive_onnx_outdir = gr.Textbox(
                                label="Output folder name",
                                value="stable-diffusion-v1-5",
                                elem_id="olive_onnx_outdir",
                            )

                            with gr.Column(elem_id="olive_onnx_res"):
                                olive_onnx_sync_hw = gr.Checkbox(
                                    label="Sync height and width (RECOMMENDED)",
                                    value=True,
                                    elem_id="olive_onnx_sync_hw",
                                )
                                olive_onnx_sample_size = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Image Size",
                                    value=512,
                                    visible=olive_onnx_sync_hw.value,
                                    elem_id="olive_onnx_sample_size",
                                )
                                olive_onnx_sample_height = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Height",
                                    value=512,
                                    visible=not olive_onnx_sync_hw.value,
                                    elem_id="olive_onnx_sample_height",
                                )
                                olive_onnx_sample_width = gr.Slider(
                                    minimum=256,
                                    maximum=2048,
                                    step=64,
                                    label="Width",
                                    value=512,
                                    visible=not olive_onnx_sync_hw.value,
                                    elem_id="olive_onnx_sample_width",
                                )

                                olive_onnx_sync_hw.change(
                                    onchange,
                                    inputs=[olive_onnx_sync_hw],
                                    outputs=[
                                        olive_onnx_sample_size,
                                        olive_onnx_sample_height,
                                        olive_onnx_sample_width,
                                    ],
                                )

                        with gr.Tab(label="Merge Extra Networks"):
                            olive_merge_lora = gr.Checkbox(
                                label="Merge LoRA",
                                value=False,
                                elem_id="olive_merge_lora",
                            )

                            olive_merge_lora_inputs = []
                            for i in range(0, 3):
                                olive_merge_lora_inputs.append(
                                    gr.Textbox(
                                        label=f"LoRA File Name {i}",
                                        value="",
                                        elem_id=f"olive_merge_lora{i}",
                                    )
                                )

            with gr.Tabs(elem_id="olive_common_tabs"):
                with gr.Tab(label="Stable Diffusion") as olive_tab_sd:
                    with FormRow(
                        elem_id="olive_ckpt_submodels",
                        elem_classes="checkboxes-row",
                        variant="compact",
                    ):
                        olive_sd_safety_checker = gr.Checkbox(
                            label="Safety Checker",
                            value=True,
                            elem_id="olive_sd_safety_checker",
                        )
                        olive_sd_text_encoder = gr.Checkbox(
                            label="Text Encoder",
                            value=True,
                            elem_id="olive_sd_text_encoder",
                        )
                        olive_sd_unet = gr.Checkbox(
                            label="UNet",
                            value=True,
                            elem_id="olive_sd_unet",
                        )
                        olive_sd_vae_decoder = gr.Checkbox(
                            label="VAE Decoder",
                            value=True,
                            elem_id="olive_sd_vae_decoder",
                        )
                        olive_sd_vae_encoder = gr.Checkbox(
                            label="VAE Encoder",
                            value=True,
                            elem_id="olive_sd_vae_encoder",
                        )

                with gr.Tab(label="Stable Diffusion XL") as olive_tab_sdxl:
                    with FormRow(
                        elem_id="olive_ckpt_submodels",
                        elem_classes="checkboxes-row",
                        variant="compact",
                    ):
                        olive_sdxl_text_encoder = gr.Checkbox(
                            label="Text Encoder",
                            value=True,
                            elem_id="olive_sdxl_text_encoder",
                        )
                        olive_sdxl_text_encoder_2 = gr.Checkbox(
                            label="Text Encoder 2",
                            value=True,
                            elem_id="olive_sdxl_text_encoder_2",
                        )
                        olive_sdxl_unet = gr.Checkbox(
                            label="UNet",
                            value=True,
                            elem_id="olive_sdxl_unet",
                        )
                        olive_sdxl_vae_decoder = gr.Checkbox(
                            label="VAE Decoder",
                            value=True,
                            elem_id="olive_sdxl_vae_decoder",
                        )
                        olive_sdxl_vae_encoder = gr.Checkbox(
                            label="VAE Encoder",
                            value=True,
                            elem_id="olive_sdxl_vae_encoder",
                        )

                with FormRow(elem_classes="checkboxes-row", variant="compact"):
                    olive_use_fp16 = gr.Checkbox(
                        label="Use half floats",
                        value=True,
                        elem_id="olive_use_fp16",
                    )

            button_olive = gr.Button(
                value="Optimize model using Olive",
                variant="primary",
                elem_id="olive_optimize",
            )

            def optimize(
                olive_checkpoint,
                olive_ckpt_vae,
                olive_ckpt_vae_subfolder,
                olive_ckpt_source_dir,
                olive_ckpt_outdir,
                olive_onnx_model_id,
                olive_onnx_vae,
                olive_onnx_vae_subfolder,
                olive_onnx_indir,
                olive_onnx_outdir,
                olive_sdxl_text_encoder,
                olive_sdxl_text_encoder_2,
                olive_sdxl_unet,
                olive_sdxl_vae_decoder,
                olive_sdxl_vae_encoder,
                olive_sd_safety_checker,
                olive_sd_text_encoder,
                olive_sd_unet,
                olive_sd_vae_decoder,
                olive_sd_vae_encoder,
                olive_ckpt_sampling_method,
                olive_use_fp16,
                olive_ckpt_sample_size,
                olive_merge_lora,
                *olive_merge_lora_inputs,
            ...
            olive_tab_ckpt.select(
                onselect_ckpt,
                inputs=[],
                outputs=[],
            )
            olive_tab_onnx.select(
                onselect_onnx,
                inputs=[],
                outputs=[],
            )
